...
export default function TestimonialList({
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error("Failed to delete testimonial or unauthorized");
    }
  };
      ...
      <div className="grid gap-4">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-border rounded-3xl bg-muted/30">
            <IoChatboxEllipses className="w-12 h-12 text-muted-foreground/30 mb-4" />
          ...
          items.map((item) => (
            ...
            >
              <div className="space-y-2 max-w-3xl">
                ...
                <p className="text-sm text-primary/80 font-semibold">{item.role}</p>
                <p className="text-sm text-muted-foreground line-clamp-2">{item.quote_en}</p>
                <p className="text-sm text-muted-foreground line-clamp-2" dir="rtl">{item.quote_ar}</p>
              </div>

              {isAdmin && (
                <div className="flex items-center gap-2 mt-4 sm:mt-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Link
                    href={`/${locale}/dashboard/testimonials/${item.id}`}
