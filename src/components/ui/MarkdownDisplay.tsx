...
export function MarkdownDisplay({ content, className }: MarkdownDisplayProps) {
  return (
    <article className={cn("max-w-none text-foreground px-4 py-8 lg:px-6 lg:py-12", className)}>
      <ReactMarkdown
        components={{
          h1: ({ node, ...props }) => <h1 className="text-4xl lg:text-5xl font-extrabold text-primary mt-12 mb-6" {...props} />,
          h2: ({ node, ...props }) => (
            <h2 className="text-3xl lg:text-4xl font-bold border-b-2 border-primary/20 pb-2 mt-12 mb-8" {...props} />
          ),
          ...
          ul: ({ node, ...props }) => <ul className="mt-6 mb-6 ml-6 pl-4 list-disc list-outside space-y-2" {...props} />,
          ol: ({ node, ...props }) => <ol className="mt-6 mb-6 ml-6 pl-4 list-decimal list-outside space-y-2" {...props} />,
          li: ({ node, ...props }) => <li className="text-foreground" {...props} />,
          blockquote: ({ node, ...props }) => (
            <blockquote className="border-l-4 border-primary pl-6 py-2 italic bg-background/50 rounded-r-lg text-foreground mt-6 mb-6" {...props} />
          ),
          img: ({ node, ...props }) => (
            // eslint-disable-next-line @next/next/no-img-element
            <img className="rounded-lg shadow-md mt-6 mb-6" alt={props.alt || ""} {...props} />
          ),
          hr: ({ node, ...props }) => <hr className="mt-10 mb-10 border-t-2 border-border/70" {...props} />,
          table: ({ node, ...props }) => (
            <div className="mt-8 mb-8 w-full overflow-y-auto">
