...
class CleanupService {
    ...
    constructor(config) {
        this.config = config;
        this.pgClient = new pg_1.Client({ connectionString: config.databaseUrl });
    ...
    async cleanupOldBackups(keepCount = 3) {
        ...
        try {
            ...
            const result = await this.pgClient.query(`SELECT id, path, "completed_at" 
         FROM "backup_logs" 
         ...
         ORDER BY "completed_at" DESC`);
            ...
            for (const backup of backupsToDelete) {
                try {
                    ...
                    await this.pgClient.query(`DELETE FROM "backup_logs" WHERE id = $1`, [backup.id]);
                    deletedCount++;
